<template>
  <v-main app>
    <div id="background">
      <div class="myform">
        <h2 class="input">Update Demogroup</h2>
        <form @submit.prevent="onSubmit">
          ...
          <div class="input">
            <label for="account">Number of Account</label>
            <input type="account" id="account" @blur="$v.account.$touch()" v-model="account" />
          </div>
          <p id="error" v-if="$v.account.$error">Please Enter Valid Number</p>
...
export default {
  ...
  methods: {
    onSubmit() {
      ...
      this.$store.dispatch('updategroup', formData);
    }
  }
};
</script>
