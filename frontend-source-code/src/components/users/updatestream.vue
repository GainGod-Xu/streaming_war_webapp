<template>
  <v-main app>
    <div id="background">
      <div class="myform">
        <h2 class="input">Update Stream</h2>
        <form @submit.prevent="onSubmit">
          ...
          <div class="input">
            <label for="subscription">Subscription Price</label>
            <input type="subscription" id="subscription" @blur="$v.subscription.$touch()" v-model="subscription" />
          </div>
          <p id="error" v-if="$v.subscription.$error">Please Enter Valid Number</p>
...
export default {
  ...
  methods: {
    onSubmit() {
      ...
      this.$store.dispatch('updatestream', formData);
    }
  }
};
</script>
